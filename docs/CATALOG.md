...
| [`lgtm-git`](../skills/lgtm-git/SKILL.md) | Create a private GitHub repo and run the release-sync git workflow used across the lgtm projects: extract a versioned `name_rNN.x.tar.gz` over the working tree, then commit, push, and watch CI in one shot, following the project's Conventional Commits convention. |
