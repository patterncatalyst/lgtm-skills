...
0. **Select a theme.** Check `themes/` for available theme JSON files. If more
   than `default.json` exists, ask the user which theme to apply — show the theme
   name and `sourceUpdated` date for each option (e.g., "Red Hat Standard
   (Apr 2026)" vs. "Designing Cloud-Native APIs (default)"). If only `default.json`
   is present, proceed silently with the default. Set the environment variable
   `DECK_THEME` to the chosen theme file path before running `node deck.js`:
   ```bash
   export DECK_THEME=./themes/red-hat-standard.json
   ```
   If no theme is selected or `DECK_THEME` is unset, `deck-helpers.js` uses its
   hardcoded constants (the original default behavior).

0b. **Product logo (optional).** Check for a `logos/` directory in the working
   folder. The structure is typically `logos/<Product Name>/` with variants:
   - `Logo-Red_Hat-<Product>-A-Standard-RGB.svg` — full-color standard (best for white slides)
   - `Logo-Red_Hat-<Product>-A-Reverse-RGB.svg` or `-White-RGB.svg` — for dark/red backgrounds
   - `..._transparent.png` — PNG with transparent background (Small/Medium/Large)

   List the product subdirectories and ask the user which product logo to use.
   Then select the appropriate variant:
   - For **cover slides** or **footer** on white backgrounds → use `Standard-RGB` (SVG or Medium PNG)
   - For **section dividers** (red/dark backgrounds) → use `Reverse-RGB` or `White-RGB`

   Suggest a placement based on the deck topic:
   - **Product-focused deck** (e.g., "OpenShift Operators") → cover + footer
   - **General Red Hat deck** with product mentions → cover only
   - **Multi-product overview** → skip, or use logos on specific slides only

   Confirm your suggestion with the user, then set:
   ```bash
   export DECK_PRODUCT_LOGO="./logos/Red Hat OpenShift/Logo-Red_Hat-OpenShift-A-Standard-RGB.svg"
   export DECK_PRODUCT_LOGO_PLACEMENT=cover   # or "footer" or "topRight"
   ```
   If no `logos/` directory exists or the user declines, skip this step.
...
- `themes/` — theme JSON files. `default.json` ships empty overrides (uses
  hardcoded constants). Additional themes are generated by the
  `lgtm-presentation-templates` skill from SVG exports of official Red Hat decks.
